...
export default function MountainBackground() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      setBackgroundOpacity(1 - progress * 0.52);
    };
    onScroll();
  ...
  return (
    <div
      style={{
        ...
        zIndex: 0,
        pointerEvents: "none",
        opacity: isMobile ? Math.min(backgroundOpacity, 0.42) : backgroundOpacity,
        transition: "opacity 180ms ease-out",
      }}
    >
      <Canvas
        camera={{ position: [0, isMobile ? -1 : -2, isMobile ? 38 : 32], fov: isMobile ? 62 : 70 }}
      ...
      >
        ...
        <Stars count={isMobile ? 600 : 1200} />
        <Suspense fallback={null}>
          <MountainWithFallback scrollRef={scrollRef} />
        </Suspense>
        <Snowfall count={isMobile ? 180 : 520} />
        {!isMobile && <HangGlider scrollRef={scrollRef} />}
      </Canvas>
    </div>
  );
}
